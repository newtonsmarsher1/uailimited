<?php
echo "🗄️ UAI Agency - Final Database Migration\n\n";
...
try {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS kcb_transfers (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        amount DECIMAL(10,2) NOT NULL,
    ...
    $pdo->exec("INSERT IGNORE INTO admin_users (username, password, role) VALUES ('admin', '$hashedPassword', 'super_admin')");
    ...
    foreach ($sampleTasks as $task) {
        $stmt = $pdo->prepare("INSERT IGNORE INTO tasks (title, description, reward, type) VALUES (?, ?, ?, ?)");
        $stmt->execute($task);
    }
    ...
    foreach ($levels as $level) {
        $stmt = $pdo->prepare("INSERT IGNORE INTO levels (level_number, name, min_tasks, min_earnings, bonus_percentage) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute($level);
    }
