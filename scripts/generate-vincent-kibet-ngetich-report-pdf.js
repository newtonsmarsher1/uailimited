...
async function generateVincentKibetNgetichReportPDF() {
  ...
  try {
    console.log('📊 Generating Report for Vincent Kibet Ngetich\n');
    ...
    const [user] = await pool.query(`
      ...
      FROM users 
      WHERE name LIKE '%Vincent%Kibet%Ngetich%'
    `);
    
    if (user.length === 0) {
      console.log('❌ User Vincent Kibet Ngetich not found');
      return;
    }
    ...
    const fileName = `${userData.name.replace(/[^a-zA-Z0-9]/g, '_')}_Report_${timestamp}.pdf`;
...
generateVincentKibetNgetichReportPDF();
