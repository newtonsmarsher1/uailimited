...
async function generateSpecificUserReportsPDF() {
  ...
  try {
    console.log('📊 Generating PDF Reports for Specific Users\n');
    
    // Users to generate reports for
    const usersToReport = [
      { id: 117, name: 'Levice Ochieng', phone: '0713478490' },
      { id: 252, name: 'Brian Memba kebaso', phone: '+254798921142' },
      { id: 194, name: 'George Linox', phone: '0746438478' },
      { id: 143, name: 'Davies kind', phone: '0106736692' },
      { id: 60, name: 'Danfas Ondieki Okoth', phone: '0727379570' }
    ];
    ...
    for (const userInfo of usersToReport) {
      console.log(`\n📋 Generating report for: ${userInfo.name} (ID: ${userInfo.id})`);
      ...
      console.log(`   Found: ${userData.name} (${userData.phone}) - Level ${userData.level}`);
      ...
      `, [userData.id, userData.invitation_code]);
      ...
      const fileName = `${userData.name.replace(/[^a-zA-Z0-9]/g, '_')}_Account_Report_${new Date().toISOString().slice(0,10)}.pdf`;
      const filePath = path.join(exportsDir, fileName);
      
      doc.pipe(fs.createWriteStream(filePath));
      
      // Header
      doc.fontSize(20).text(`${userData.name.toUpperCase()} - ACCOUNT REPORT`, { align: 'center' });
      doc.moveDown();
      ...
      doc.text(`Activity Rate: ${daysSinceJoining > 0 ? ((activeDays / daysSinceJoining) * 100).toFixed(1) : 0}%`);
      doc.text(`Total Tasks Completed: ${completedTasks.length}`);
      doc.text(`Average Tasks per Active Day: ${avgTasksPerDay}`);
      doc.text(`Average Tasks per Day (overall): ${daysSinceJoining > 0 ? (completedTasks.length / daysSinceJoining).toFixed(2) : 0}`);
      doc.text(`Total Task Earnings: KES ${taskEarnings.toFixed(2)}`);
      doc.text(`Average Earnings per Active Day: KES ${avgEarningsPerDay}`);
      doc.text(`Average Earnings per Day (overall): KES ${daysSinceJoining > 0 ? (taskEarnings / daysSinceJoining).toFixed(2) : 0}`);
      doc.moveDown();
    ...
    console.log(`\n📁 All reports saved in: ${exportsDir}`);
...
generateSpecificUserReportsPDF();
