...
async function generatePhoneReportPDF() {
  ...
  try {
    console.log('📊 Generating Report for Phone Number: 0746438478\n');
    ...
    const [user] = await pool.query(`
      ...
      FROM users 
      WHERE phone = '0746438478'
    `);
    
    if (user.length === 0) {
      console.log('❌ User with phone number 0746438478 not found');
      return;
    }
