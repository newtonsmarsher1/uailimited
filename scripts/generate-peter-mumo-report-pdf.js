...
async function generatePeterMumoReportPDF() {
  ...
  try {
    console.log('📊 Generating Report for Peter Mumo\n');
    ...
    const [user] = await pool.query(`
      ...
      FROM users 
      WHERE name LIKE '%Peter%Mumo%'
    `);
    
    if (user.length === 0) {
      console.log('❌ User Peter Mumo not found');
      return;
    }
