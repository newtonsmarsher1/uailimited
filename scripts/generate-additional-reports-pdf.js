...
async function generateAdditionalReportsPDF() {
  ...
  try {
    console.log('📊 Generating Reports for Additional Users\n');
    
    // Users to generate reports for
    const usersToReport = [
      { name: 'Antony Momanyi', searchPattern: '%Antony%Momanyi%' },
      { name: 'Vincent Kibet', searchPattern: '%Vincent%Kibet%' },
      { name: 'Sammy Okoth', searchPattern: '%Sammy%Okoth%' },
      { name: 'Peter Hezron', searchPattern: '%Peter%Hezron%' }
    ];
    
    for (const userInfo of usersToReport) {
      console.log(`\n📋 Generating report for: ${userInfo.name}`);
      ...
      doc.text(`Joined Date: ${new Date(userData.created_at).toLocaleDateString()}`);
