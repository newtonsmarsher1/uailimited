...
async function generateGeorgeLinuxReportPDF() {
  ...
  try {
    console.log('📊 Generating Report for George Linux\n');
    ...
    const [user] = await pool.query(`
      ...
      FROM users 
      WHERE name LIKE '%George%Linux%'
    `);
    
    if (user.length === 0) {
      console.log('❌ User George Linux not found');
      return;
    }
