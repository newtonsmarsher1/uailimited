...
async function generateFrancisReportPDF() {
  ...
  try {
    console.log('📊 Generating Francis Shiundu\'s Report in PDF Format\n');
    
    // Get Francis's details
    const [user] = await pool.query(`
      ...
      FROM users 
      WHERE name LIKE '%Francis%Shiundu%' OR name LIKE '%Francis%Shiundu%'
    `);
    ...
    console.log(`📋 Generating report for: ${userData.name}`);
    ...
    console.log(`📁 Report saved in: ${exportsDir}`);
