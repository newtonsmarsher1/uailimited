...
async function generateDetailedReportsPDF() {
  ...
  try {
    console.log('📊 Generating Detailed Reports Since Join Date\n');
    
    // Users to generate reports for
    const usersToReport = [
      { name: 'Eliezer Magati', searchPattern: '%Eliezer%Magati%' },
      { name: 'Charles Ochoi', searchPattern: '%Charles%Ochoi%' },
      { name: 'Davies Kind', searchPattern: '%Davies%Kind%' }
    ];
    
    for (const userInfo of usersToReport) {
      console.log(`\n📋 Generating detailed report for: ${userInfo.name}`);
      ...
      const fileName = `${userData.name.replace(/[^a-zA-Z0-9]/g, '_')}_Detailed_Report_${new Date().toISOString().slice(0,10)}.pdf`;
      const filePath = path.join(exportsDir, fileName);
      
      doc.pipe(fs.createWriteStream(filePath));
      
      // Header
      doc.fontSize(20).text(`${userData.name.toUpperCase()} - DETAILED REPORT`, { align: 'center' });
      doc.moveDown();
      ...
      console.log(`✅ Detailed PDF report generated: ${fileName}`);
    }
    
    console.log(`\n📁 All reports saved in: ${path.join(process.cwd(), 'exports')}`);
