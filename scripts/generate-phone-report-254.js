...
async function generatePhoneReport254PDF() {
  ...
  try {
    console.log('📊 Generating Report for Phone Number: +254719344467\n');
    ...
    const [user] = await pool.query(`
      ...
      FROM users 
      WHERE phone = '+254719344467'
    `);
    
    if (user.length === 0) {
      console.log('❌ User with phone number +254719344467 not found');
      return;
    }
