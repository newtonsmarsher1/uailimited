...
router.post('/withdraw', simpleAuth, async (req, res) => {
  try {
    ...
    const [user] = await pool.query(
      'SELECT wallet_balance, withdrawal_pin, pin_attempts, pin_locked_until, full_name, id_number, bank_type, account_number FROM users WHERE id = ?',
      [req.user.id]
    );
    ...
    console.log('📊 User data:', {
      ...
      has_id_number: !!userData.id_number,
    ...
    // Check if user has set ID number (required for first withdrawal)
    if (!userData.id_number) {
      console.log('❌ User has not set ID number');
      return res.status(400).json({ 
        error: 'Please enter your ID number to complete your first withdrawal.',
    ...
    if (isSecondTuesday) {
      console.log('❌ Withdrawal attempted on second Tuesday (auditing day)');
      return res.status(400).json({ 
...
router.post('/recharge-method1', simpleAuth, async (req, res) => {
  try {
    console.log('🔍 Recharge method 1 request received:', {
      userId: req.user.id,
      body: req.body
    });
    ...
    if (!amount || !phone) {
      console.log('❌ Missing required fields:', { amount, phone });
    ...
    if (isNaN(amount) || amount <= 0) {
      ...
      return res.status(400).json({ error: 'Invalid amount. Please enter a valid positive number.' });
    }
    
    // Get CEO-added financial details for method 1
    console.log('🔍 Fetching financial details for method1...');
    ...
    console.log('📊 Financial details query result:', financialDetails);
    
    if (!financialDetails.length) {
      console.log('❌ No financial details found for method1');
    ...
    console.log('✅ Financial manager found:', financialDetail.name);
    ...
    try {
      ...
      console.log('⚠️ User notification failed:', notificationError.message);
    }
    ...
    try {
      ...
      console.log('✅ Financial manager notification sent');
    } catch (notificationError) {
      console.log('⚠️ Financial manager notification failed:', notificationError.message);
    }
    
    console.log('🎉 Recharge method 1 completed successfully');
    res.json({ 
      success: true,
    ...
    console.error('❌ Error processing recharge method 1:', error);
    console.error('❌ Error stack:', error.stack);
    res.status(500).json({ 
...
router.post('/recharge-method2', simpleAuth, async (req, res) => {
  try {
    console.log('🔍 Recharge method 2 request received:', {
      userId: req.user.id,
      body: req.body
    });
    ...
    console.log('🔍 Fetching financial details for method2...');
    const [financialDetails] = await pool.query(`
      SELECT id, name, mobile, bank_name, account_number, branch, swift_code, reference_code
      FROM admin_users 
    ...
    if (!financialDetails.length) {
      console.log('❌ No financial details found for method2');
      return res.status(500).json({ 
    ...
    console.log('🎉 Recharge method 2 completed successfully');
    res.json({ 
      success: true,
      financialManagerName: financialDetail.name,
      transactionNumber: transactionNumber,
...
router.post('/recharge-method3', simpleAuth, async (req, res) => {
  try {
    console.log('🔍 Recharge method 3 request received:', {
      userId: req.user.id,
      body: req.body
    });
    ...
    console.log('🔍 Fetching HR manager for method3...');
    const [hrManagers] = await pool.query(`
    ...
    console.log('📊 HR managers query result:', hrManagers);
    
    if (!hrManagers.length) {
      console.log('❌ No HR managers found for method3');
      return res.status(500).json({ error: 'No HR managers available at the moment' });
    }
    
    const hrManager = hrManagers[0];
    console.log('✅ HR manager found:', hrManager.name);
    ...
    try {
      ...
      console.log('✅ HR manager notification sent');
    } catch (notificationError) {
      console.log('⚠️ HR manager notification failed:', notificationError.message);
    }
    
    console.log('🎉 Recharge method 3 completed successfully');
    res.json({ 
      success: true,
...
router.post('/recharge-method4', simpleAuth, async (req, res) => {
  try {
    console.log('🔍 Recharge method 4 request received:', {
      userId: req.user.id,
      body: req.body
    });
    ...
    // Get a random HR manager from admin_users table
    console.log('🔍 Fetching HR manager for method4...');
    const [hrManagers] = await pool.query(`
      SELECT id, name, mobile as phone FROM admin_users 
      WHERE verified = 1 AND role != 'CEO' AND rejected = 0
      ORDER BY RAND() 
    ...
    if (!hrManagers.length) {
      console.log('❌ No HR managers found for method4');
      return res.status(500).json({ error: 'No HR managers available at the moment' });
    }
    ...
    console.log('🎉 Recharge method 4 completed successfully');
    res.json({ 
      success: true,
      hrName: hrManager.name,
      transactionNumber: transactionNumber,
