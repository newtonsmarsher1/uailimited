...
async function verifyDatabase() {
  try {
    console.log('🔍 Verifying PostgreSQL database connection and tables...');
    ...
    if (!paymentsTable.rows[0].exists) {
    ...
    const withdrawalsTable = await client.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_schema = 'public' 
    ...
    if (!withdrawalsTable.rows[0].exists) {
    ...
    const appealsTable = await client.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_schema = 'public' 
